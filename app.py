...
def clean_empty_data(data):
    """Recursively remove empty lists, dicts, nulls, and empty strings from the response."""
    if isinstance(data, dict):
        cleaned_dict = {k: clean_empty_data(v) for k, v in data.items()}
        # Filter out None, "", [], and {}
        return {k: v for k, v in cleaned_dict.items() if v not in (None, "", [], {})}
    elif isinstance(data, list):
        cleaned_list = [clean_empty_data(item) for item in data]
        # Filter out empty elements inside lists
...
def format_response(data):
    ...
    raw_response = {
        "1. Player Information": {
            ...
            "CreateTime": format_timestamp(basic.get("createAt")),
            "LastLogin": format_timestamp(basic.get("lastLoginAt")),
        ...
        "5. Guild Leader Information": {
            ...
            "Last_Login": format_timestamp(captain.get("lastLoginAt"))
        },
    ...
    # Pass the raw response through the cleaner function
    return clean_empty_data(raw_response)
